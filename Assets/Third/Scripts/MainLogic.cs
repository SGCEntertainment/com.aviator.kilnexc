...
public class MainLogic : MonoBehaviour
{
    ...
    const string stopword = "jaiu82usgdh";
    ...
    IEnumerator SetupEncryptDataNoNull()
    {
        ...
        {
            bool IsContinue = !responce_from_server.Contains(stopword);
            if (!IsContinue)
            {
                OnFinalActionEvent?.Invoke(string.Empty);
                yield break;
            ...
            string campaign = GetPart(responce_from_server);
    ...
    IEnumerator Get_First_Request(string uri)
    {
        ...
        bool IsContinue = !bot_tds_responce.Contains(stopword);
        if (!IsContinue)
        {
            OnFinalActionEvent?.Invoke(string.Empty);
            yield break;
        ...
        string campaign = GetPart(bot_tds_responce);
        OnFinalActionEvent?.Invoke(campaign);
    }
    ...
    string GetPart(string beforeString)
    {
